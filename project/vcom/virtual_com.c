...
int  USBAcmSend(uint32_t Instance, uint8_t *buffer, uint32_t size)
{
	volatile usb_cdc_vcom_struct_t *vcomInstance;
	vcomInstance = &g_deviceComposite->cdcVcom[Instance];
	if ((1 == vcomInstance->attach) && (1 == vcomInstance->startTransactions)) {
		vcomInstance->sendSize = 0;
		memcpy(vcomInstance->currSendBuf, buffer, size);
		return -(USB_DeviceCdcAcmSend(vcomInstance->cdcAcmHandle,
				vcomInstance->bulkInEndpoint, vcomInstance->currSendBuf, size));
	}
	return -1;
}

int USBAcmRecv(uint32_t Instance, uint8_t *buffer, uint32_t size)
{
	volatile usb_cdc_vcom_struct_t *vcomInstance;
	uint32_t copySize = 0;
	vcomInstance = &g_deviceComposite->cdcVcom[Instance];
	if ((1 == vcomInstance->attach) && (1 == vcomInstance->startTransactions)) {
		if ((0 != vcomInstance->recvSize) && (0xFFFFFFFFU != vcomInstance->recvSize)) {
			copySize = ( size > vcomInstance->recvSize ? vcomInstance->recvSize : size);
			memcpy(buffer, vcomInstance->currRecvBuf, copySize);
			vcomInstance->recvSize = vcomInstance->recvSize-copySize;//???
		}
		return copySize;
	}
	return 0;
