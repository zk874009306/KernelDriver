...
static void MSP432_SPI_Send_Data(unsigned char *TxBuffer, unsigned int num)
{
    //unsigned char RxBuffer;
    spiTransaction.count = num;
...
static void MSP432_SPI_Get_Data(unsigned char *RxBuffer, unsigned int num)
{
    //unsigned char RxBuffer;
    spiTransaction.count = num;
...
uint8_t spi_sx1276_master_init( MSP_EXP432P401R_SPIName SPIPort )
{
//    GPIO_init();
    ...
    spiParams.bitRate = 1000000;//set to 1MHz
    spiParams.frameFormat = SPI_POL0_PHA0;
    spi = SPI_open(SPIPort, &spiParams);
    if (spi == 0) {
        return 1;  // SPI_open() failed
    }
    return 0;
}
...
extern void
LPC54018_SPI_Send_Data(unsigned char *TxBuffer, unsigned int num);
extern void
LPC54018_SPI_Get_Data(unsigned char *RxBuffer, unsigned int num);
...
void SX1276_WriteReg(INT8U addr, INT8U value)
{
    ...
    TxBuffer = addr | 0x80;
    SX1276_NSS_LOW( );
    LPC54018_SPI_Send_Data(&TxBuffer, 1);
    LPC54018_SPI_Send_Data(&value, 1);
    SX1276_NSS_HIGH( );
//    log_notice("write add = %x\r\n", TxBuffer);
}
/*
 ===============================================================================
...
void SX1276_WriteBuffer(INT8U addr, INT8U *buffer, INT8U size)
{
    ...
    SX1276_NSS_LOW( );
    LPC54018_SPI_Send_Data(&TxBuffer, 1);
    LPC54018_SPI_Send_Data(buffer, size);
    SX1276_NSS_HIGH( );
//    log_notice("write add = %x\r\n", TxBuffer);
}
/*
...
INT8U SX1276_ReadReg(INT8U addr)
{
    ...
    SX1276_NSS_LOW( );
    LPC54018_SPI_Send_Data(&TxBuffer, 1);
    LPC54018_SPI_Get_Data(&RxBuffer, 1);
    SX1276_NSS_HIGH( );
//    log_notice("read add = %x, value=%x\r\n", TxBuffer, RxBuffer);
    return RxBuffer;
}
/*
 ===============================================================================
 ...
 *Output  :  The read value
 ===============================================================================
 */
void SX1276_ReadFifoBuffer(INT8U *buffer, INT8U size)
{
    unsigned char TxBuffer;
//        unsigned char RxBuffer;
    TxBuffer = 0x00 & 0x7F;
    SX1276_NSS_LOW( );
    LPC54018_SPI_Send_Data(&TxBuffer, 1);
    LPC54018_SPI_Get_Data(buffer, size);
    SX1276_NSS_HIGH( );
    //    log_notice("read add = %x, value=%x\r\n", TxBuffer, RxBuffer);
//        return RxBuffer;
}
 ...
 *Output  :  None
 ===============================================================================
 */
static const INT8U Freq_Table[][3] = { { 0x66, 0x80, 0x00 },          //410,  0
...
{
    if ((SX1276.Mode_Lora_Fsk != MODE_FSK)
            && (SX1276.Mode_Lora_Fsk != MODE_LORA)) {
        SX1276.Mode_Lora_Fsk = MODE_LORA;
    }
...
void SX1276_CADMode(void)
{
    ...
    SX1276_WriteReg(
    REG_LR_IRQFLAGSMASK,
            DEF_LR_IRQFLAGSMASK | RFLR_IRQFLAGS_RXTIMEOUT | RFLR_IRQFLAGS_RXDONE
    ...
    SX1276_WriteReg( REG_LR_DIOMAPPING1,
    DEF_LR_DIOMAPPING1 | RFLR_DIOMAPPING1_DIO0_00  //RxDone
            | RFLR_DIOMAPPING1_DIO1_00  //RxTimeout
            | RFLR_DIOMAPPING1_DIO2_00  //FhssChangeChannel
            | RFLR_DIOMAPPING1_DIO3_00);  //CadDone
    SX1276_WriteReg( REG_LR_DIOMAPPING2,
    DEF_LR_DIOMAPPING2 | RFLR_DIOMAPPING2_DIO4_00  //CadDetected
...
//	    power = SX1276.PowerIndex;
    ...
    {
        SX1276_LoRa_SetMode(LORA_MODE_STDBY); //standbyģʽ�����޸�ģ��Ĺ��ʣ��ι��ʿ��Է���ģ���ʼ����ʱ��
        tmp = SX1276_ReadReg(0x4D);
        tmp &= ~((1 << 2) | (1 << 1) | (1 << 0));
        tmp |= (0x07 << 0);                        //4d�Ĵ���ֱ��дΪ���
        SX1276_WriteReg(0x4D, tmp);
        tmp = SX1276_ReadReg(0x09);
        tmp |= (1 << 7);
        tmp &= ~((1 << 3) | (1 << 2) | (1 << 1) | (1 << 0));
        tmp |= (power << 0);                       //09�Ĵ���ֱ��дΪ15
        SX1276_WriteReg(0x09, tmp);
    }
}
