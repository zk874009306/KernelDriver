...
void *nlp_create(C2CONST *c2const)
{
    ...
    nlp = (NLP*)CODEC2_MALLOC(sizeof(NLP));
    if (nlp == NULL)
	return NULL;
    ...
    if (Fs == 16000) {
        nlp->Sn16k = (float*)CODEC2_MALLOC(sizeof(float)*(FDMDV_OS_TAPS_16K + c2const->n_samp));
...
void nlp_destroy(void *nlp_state)
{
    ...
    if (nlp->Fs == 16000) {
        CODEC2_FREE(nlp->Sn16k);
    }
    CODEC2_FREE(nlp_state);
}
...
)
{
    ...
    Fw = (COMP*)CODEC2_MALLOC(sizeof(COMP)*PE_FFT_SIZE);
    assert(Fw != NULL);
    assert(nlp_state != NULL);
    nlp = (NLP*)nlp_state;
    m = nlp->m;
