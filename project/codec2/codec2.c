...
struct CODEC2 * codec2_create(int mode)
{
    ...
    c2 = (struct CODEC2*)CODEC2_MALLOC(sizeof(struct CODEC2));
    if (c2 == NULL)
	return NULL;
    ...
    c2->Pn = (float*)CODEC2_MALLOC(2*n_samp*sizeof(float));   //2*80*4
    if (c2->Pn == NULL) {
	return NULL;
    }
    c2->Sn_ = (float*)CODEC2_MALLOC(2*n_samp*sizeof(float)); //2*80*4
    if (c2->Sn_ == NULL) {
	return NULL;
    }
    c2->w = (float*)CODEC2_MALLOC(m_pitch*sizeof(float));  // 320*4
    if (c2->w == NULL) {
	return NULL;
    }
    c2->Sn = (float*)CODEC2_MALLOC(m_pitch*sizeof(float)); // 320*4
    if (c2->Sn == NULL) {
	return NULL;
    }
    ...
    c2->bpf_buf = (float*)CODEC2_MALLOC(sizeof(float)*(BPF_N+4*c2->n_samp));
...
void analyse_one_frame(struct CODEC2 *c2, MODEL *model, short speech[])
{
    ...
    Sw = (COMP*)CODEC2_MALLOC(sizeof(COMP)*FFT_ENC);
    assert(Sw != NULL);
    /* Read input speech */
